"""
This program initializes a single UDP Server which uses port number 12001. The server attempts to take the data it recieves,
...
while True: #loop forever while waiting for new packets
	try: #deal with errors without shutting down
	  msg, addr = s.recvfrom(1024) #recieve and decode UDP packets
	  s.sendto(msg.encode().upper(),addr) #capitalize recieved data and send back to source.
	except:
